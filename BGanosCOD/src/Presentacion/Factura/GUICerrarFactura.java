	...
	*/
	...
	public void initGUI() {
		...
		JLabel msgIntroIDCabecera = ComponentsBuilder.createLabel(
				"Introduzca el ID de la entrada que desea agregar a la factura ", 1, 10, 80, 20, Color.BLACK);
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (id.getText().isEmpty() || cantidadtxt.getText().isEmpty()) {
						JOptionPane.showMessageDialog(GUICerrarFactura.this, "no se han rellenado todos los campos",
						...
						ApplicationController.getInstance()
								.manageRequest(new Context(Evento.CERRAR_FACTURA_VISTA, tCarrito));
					}
			...
			public void actionPerformed(ActionEvent e) {
				try {
					if (idOut.getText().isEmpty() || cantidadOuttxt.getText().isEmpty()) {
						...
						if (!encontrado) {
							ApplicationController.getInstance()
									.manageRequest(new Context(Evento.CERRAR_FACTURA_KO, -1));
						} else if (!correct) {
							ApplicationController.getInstance()
									.manageRequest(new Context(Evento.CERRAR_FACTURA_KO, -3));
						} else {
							GUICerrarFactura.this.setVisible(false);
							tCarrito.setLineaFactura(lineaFacturas);
							ApplicationController.getInstance()
