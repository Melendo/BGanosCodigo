...
public class GUIAltaPlanta extends JFrame implements IGUI {
	...
	*/
	...
	public void initGUI() {
		...
		JLabel msgIntroIDCabecera = ComponentsBuilder.createLabel("Seleccione el tipo de planta que desea dar de alta",
		...
		JLabel labelnombrecientifico = ComponentsBuilder.createLabel("Nombre Cientifico: ", 10, 100, 80, 20,
				Color.BLACK);
				...
				try {
					...
					if (tipoPlanta.getSelectedItem() == "Frutal") {
						...
						if (fruta.isEmpty() || mad.isEmpty() || nombre.isEmpty() || nombreCientifico.isEmpty())
							...
							TPlantaFrutal frutal = new TPlantaFrutal(nombre, nombreCientifico, tipo, idInvernadero,
									fruta, mad);
						...
						if (hoja.isEmpty() || nombre.isEmpty() || nombreCientifico.isEmpty())
							GUIMSG.showMessage("Rellene los campos", "ALTA PLANTA", true);
						else {
							TPlantaNoFrutal NOfrutal = new TPlantaNoFrutal(nombre, nombreCientifico, tipo,
									idInvernadero, hoja);

							ApplicationController.getInstance()
									.manageRequest(new Context(Evento.ALTA_PLANTA, NOfrutal));
						}
