	...
	*/
	...
	private void buscarPorFabricante() {
		...
		if (fabricante.isEmpty()) {
			JOptionPane.showMessageDialog(this, "Por favor, ingrese un fabricante.", "Advertencia",
		...
		try {
			...
			ApplicationController.getInstance()
					.manageRequest(new Context(Evento.LISTAR_SISTEMAS_RIEGO_POR_FABRICANTE, idFabricante));
		} catch (NumberFormatException ex) {
			JOptionPane.showMessageDialog(this, "Por favor, ingrese un número válido para el ID del fabricante.",
	...
	public void actualizar(Context context) {
		if (context.getEvento() == Evento.LISTAR_SISTEMA_DE_RIEGO_POR_FABRICANTE_OK) {
			...
			for (TSistemaDeRiego sistema : sistemas) {
				datos[i++] = new String[] { String.valueOf(sistema.getId()), sistema.getNombre(),
						String.valueOf(sistema.getPotenciaRiego()), String.valueOf(sistema.getCantidad_agua()),
						String.valueOf(sistema.getFrecuencia()), sistema.getActivo() ? "Si" : "No",
						String.valueOf(sistema.getIdFabricante()) };

			}
			tabla.setModel(new javax.swing.table.DefaultTableModel(datos, new String[] { "ID", "Nombre",
					"Potencia Riego", "Cantidad Agua", "Frecuencia", "Activo", "Fabricante" }));
			ComponentsBuilder.adjustColumnWidths(tabla);
		} else if (context.getEvento() == Evento.LISTAR_SISTEMA_DE_RIEGO_POR_FABRICANTE_KO) {
			JOptionPane.showMessageDialog(this, "Error al listar sistemas de riego.", "Error",
